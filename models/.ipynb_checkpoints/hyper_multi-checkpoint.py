...
class BaseH(KGModel):
    ...
    def similarity_score(self, lhs_e, rhs_e, eval_mode):
        lhs_e, c = lhs_e
        if self.space == 'euc':
            return euc_sqdistance(lhs_e, rhs_e, eval_mode)
...
class GIE(BaseH):
    ...
    def get_queries(self, queries):
        ...
        res1=logmap0(res1,c1).view(-1, 1, self.rank)
        res11=logmap0(res11,c2).view(-1, 1, self.rank)

        c = F.softplus(self.c[queries[:, 1]])

        head = self.entity(queries[:, 0]).view((-1, 1, self.rank))
        rot_mat, _ = torch.chunk(self.rel_diag(queries[:, 1]), 2, dim=1)
        rot_q = givens_rotations(rot_mat, head).view((-1, 1, self.rank))

        cands = torch.cat([res1,res11,rot_q], dim=1)
    ...
    def get_queries_lp(self, queries, r):
        ...
        rel21 = expmap0(rel2, c2)
        lhss = project(mobius_add(head2, rel11, c2), c2)
        res11 = givens_rotations(self.rel_diag2(r), lhss)
...
class EUC(BaseH):
    ...
    def get_queries(self, queries):
        ...
        # cands = torch.cat([head], dim=1)
        # att_q = torch.sum(cands, dim=1)
        rel, _ = torch.chunk(self.rel(queries[:, 1]), 2, dim=1)
        res = head + rel
...
class HYP(BaseH):
    ...
    def get_queries(self, queries):
        ...
        c1 = F.softplus(self.c1[queries[:, 1]])
        head1 = expmap0(self.entity(queries[:, 0]), c1)
        rel1, _ = torch.chunk(self.rel(queries[:, 1]), 2, dim=1)
        rel1 = expmap0(rel1, c1)
        lhs = project(mobius_add(head1, rel1, c1), c1)
        res1 = givens_rotations(self.rel_diag1(queries[:, 1]), lhs) 
        ...
        rel, _ = torch.chunk(self.rel(queries[:, 1]), 2, dim=1)
        rel = expmap0(rel, c1)
        res = project(mobius_add(lhs, rel, c1), c1)
        
        return (res, c1), self.bh(queries[:, 0]) # lhs biases

    def get_queries_lp(self, queries, r):
        
        c1 = F.softplus(self.c1[r])
        head1 = expmap0(self.entity(queries[:, 0]), c1)
        rel1, _ = torch.chunk(self.rel(r), 2, dim=1)
        rel1 = expmap0(rel1, c1)
        lhs = project(mobius_add(head1, rel1, c1), c1)
        res1 = givens_rotations(self.rel_diag1(r), lhs) 
...
class ATT2(BaseH):
    ...
    def get_queries(self, queries):
        ...
        cands = torch.cat([res1, rot_q], dim=1)
        context_vec = self.context_vec(queries[:, 1]).view((-1, 1, self.rank))
        att_weights = torch.sum(context_vec * cands * self.scale, dim=-1, keepdim=True)
        att_weights = self.act(att_weights)
        att_q = torch.sum(att_weights * cands, dim=1)
