...
parser.add_argument(
    '--directory', type=str, default='GO1015'
)
...
def prepare_dataset(path, name): # /src_data/GO0608, GO0608
    ...
    # map test_neg with the ids #############
    file_path = os.path.join(path, 'test_neg')
    to_read = open(file_path, 'r')
    examples = []
    for line in to_read.readlines():
        lhs, rel, rhs = line.strip().split('\t')
        try:
            ...
            continue
    out = open(Path(DATA_PATH) / name / ('test_neg' + '.pickle'), 'wb')
    pickle.dump(np.array(examples).astype('uint64'), out)
    out.close()
    
    # map test_neg_inv with the ids #############
    file_path = os.path.join(path, 'test_neg_inv')
    to_read = open(file_path, 'r')
    examples = []
    for line in to_read.readlines():
        lhs, rel, rhs = line.strip().split('\t')
        try:
            ...
            continue
    out = open(Path(DATA_PATH) / name / ('test_neg_inv' + '.pickle'), 'wb')
    pickle.dump(np.array(examples).astype('uint64'), out)
    out.close()
    
    # map test_neg_rel with the ids #############
    file_path = os.path.join(path, 'test_neg_rel')
    to_read = open(file_path, 'r')
    examples = []
    for line in to_read.readlines():
        lhs, rel, rhs = line.strip().split('\t')
        try:
            ...
            continue
    out = open(Path(DATA_PATH) / name / ('test_neg_rel' + '.pickle'), 'wb')
    pickle.dump(np.array(examples).astype('uint64'), out)
    out.close()
import os
import errno
...
if __name__ == "__main__":
    datasets = ['GO1015-is_a', 'GO1015-part_of', 'GO1015-has_part', 'GO1015-regulates', 'GO1015-positively_regulates', 'GO1015-negatively_regulates', 'GO1015-occurs_in']
    for d in datasets:
