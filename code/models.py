...
class GIE(KBCModel):
    def __init__(
        ...
        self.sizes = sizes # [43310, 16, 43310]
        self.rank = rank # 2000
    ...
    def forward(self, x):
        ...
        rel = self.embeddings[1](x[:, 1]) # batch_size, rank*2
        rhs = self.embeddings[0](x[:, 2])
        
        rel1 = self.embeddings1[0](x[:, 1])
        rel2 = self.embeddings1[1](x[:, 1])
        
        entities = self.embeddings[0].weight # (43310, rank*2)
        ...
        head1 = expmap0(lhs, c1) # H_h
        # rel11 = expmap0(rel1, c1)
        lhs = head1
        res_c1 =logmap0(givens_rotations(rel2, lhs),c1).view((-1, 1, self.rank)) # log(H_h)
        
        translation1=lhs_t[1] * rel[1]
        
        c2 = F.softplus(self.c2[x[:, 1]])
        head2 = expmap0(lhs, c2) # S_h
        # rel12 = expmap0(rel1, c2)
        lhss = head2
        res_c2 = logmap0(givens_rotations(rel2, lhss),c2).view((-1, 1, self.rank)) # log(S_h)
        
        translation2=lhs_t[1] * rel[0]
        
        c = F.softplus(self.c[x[:, 1]])
        head = lhs
        rot_q = givens_rotations(rel2, head).view((-1, 1, self.rank)) # E_h
        
        cands = torch.cat([res_c1, res_c2, rot_q], dim=1) # log(H_h), log(S_h), E_h
        
        context_vec = self.context_vec(x[:, 1]).view((-1, 1, self.rank))
        att_weights = torch.sum(context_vec * cands * self.scale, dim=-1, keepdim=True)
        att_weights = self.act(att_weights) # Lambda_H, Lambda_S, Labmda_E
        att_q = torch.sum(att_weights * cands, dim=1) # inter(H_h, S_h, E_h)
        
        return (
                    (att_q * rel[0] - translation1) @ entity1.t() +(att_q * rel[1] + translation2) @ entity2.t() # prediction
                    # (att_q * rel[0] - translation1) @ entity1.t()
               ), [
