...
trp = pd.read_csv('/home/ukjung18/GIE/GIE-master/src_data/go_kgx_tsv_edges.tsv', delimiter='\t')


# In[4]:


trp = trp[['subject', 'predicate', 'object']]
...
for i in range(num_rel):
    if (trp.loc[i]['predicate'] == 'biolink:subPropertyOf') or (trp.loc[i]['predicate'] == 'biolink:inverseOf'):
        continue
    ent.add(trp.loc[i]['subject'])
    rel.add(trp.loc[i]['predicate'])
    ent.add(trp.loc[i]['object'])
...
for i in range(0, num_rel):
    if (trp.loc[i]['predicate'] == 'biolink:subPropertyOf') or (trp.loc[i]['predicate'] == 'biolink:inverseOf'):
        continue
    u.append(ent_to_id[trp.loc[i]['subject']])
    v.append(ent_to_id[trp.loc[i]['object']])
    r.append(rel_to_id[trp.loc[i]['predicate']])
